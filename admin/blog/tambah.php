...
if(isset($_POST['tambah'])){
	...
	if ($nama_gambar!=null){
		if($tipe_gambar == "image/jpeg" || $tipe_gambar == "image/png"){ // Cek apakah tipe file yang diupload adalah JPG / JPEG / PNG  
			...
			if($ukuran_gambar <= 1000000){ // Cek apakah ukuran file yang diupload kurang dari sama dengan 1MB    
				...
				if(move_uploaded_file($tmp_gambar, $path_gambar)){
					$sql = "INSERT INTO landblog_blog (waktu, gambar, judul, deskripsi, link) 
							VALUES (:waktu, :gambar, :judul, :deskripsi, :link)";
					$stmt = $db->prepare($sql);
					$params = array(
						":waktu" => date("d F Y"),
						":gambar" => $nama_gambar,
		...
		$sql = "INSERT INTO landblog_blog (waktu, judul, deskripsi, link) 
				VALUES (:waktu, :judul, :deskripsi, :link)";
		$stmt = $db->prepare($sql);
		$params = array(
			":waktu" => date("d F Y"),
			":judul" => $judul,
...
<head>
    ...
    <title>Tambah Tulisan - Admin</title>
...
<div class="container mt-5">
    <div class="row">
        <div class="col-md-12">       
			...
			<?php }?>
			<div class="card mb-3">
				<div class="card-header text-center bg-dark text-white">Tambah Tulisan</div>
                <div class="card-body">
					<form method="POST" enctype="multipart/form-data">	
						...
						<div class="form-group">
							...
							<textarea class="form-control ckeditor" id="deskripsi" name="deskripsi" required></textarea>
						</div>
