...
if(isset($_POST['edit'])){
	...
	if ($nama_gambar!=null){
		if($tipe_gambar == "image/jpeg" || $tipe_gambar == "image/png"){ // Cek apakah tipe file yang diupload adalah JPG / JPEG / PNG  
			...
			if($ukuran_gambar <= 1000000){ // Cek apakah ukuran file yang diupload kurang dari sama dengan 1MB    
				...
				if(move_uploaded_file($tmp_gambar, $path_gambar)){
					$sql = "UPDATE landblog_blog 
							SET gambar=:gambar, judul=:judul, deskripsi=:deskripsi, link=:link
							WHERE id=:id";
					$stmt = $db->prepare($sql);
					$params = array(
						":id" => $id,
		...
		$sql = "UPDATE landblog_blog
				SET judul=:judul, deskripsi=:deskripsi, link=:link
				WHERE id=:id";
		$stmt = $db->prepare($sql);
		$params = array(
			":id" => $id,
...
<head>
    ...
    <title>Edit Tulisan - Admin</title>
...
<div class="container mt-5">
    <div class="row">
        <div class="col-md-12">     
			...
			<?php }?>
			<div class="card mb-3">
				<div class="card-header text-center bg-dark text-white">Edit Tulisan</div>
                <div class="card-body">
					...
					<?php foreach($result as $row) {?>
						...
						<div class="form-group">
							...
							<input id="judul" type="text" class="form-control" name="judul" value="<?php echo $row["judul"];?>" required autofocus>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							...
							<textarea class="form-control ckeditor" id="deskripsi" name="deskripsi" required><?php echo $row["deskripsi"];?></textarea>
						</div>
